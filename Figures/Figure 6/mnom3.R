...
GISAID = read.csv("~/Documents/R scripts/Phylodynamics/Multinomial fit databases/3rd/GISAID3_upd.csv", sep = ",")
...
range(GISAID$date) # "2021-03-07" "2021-07-01"
nrow(GISAID) #2712
...
GISAID$pango_lineage[grepl("B.1.625", GISAID$pango_lineage)]
...
# use data from March 07 2021 onwards
GISAID_sel = GISAID_sel[GISAID_sel$date>=as.Date("2021-03-07"),]
nrow(GISAID_sel) # 2712
range(GISAID_sel$date) # "2021-03-07" "2021-07-01"
...
sum(GISAID_sel$LINEAGE2==sel_target_VOC) # 4010
sum(GISAID_sel$LINEAGE2=="Lambda") # 142
sum(GISAID_sel$LINEAGE2=="Gamma")# 826
sum(GISAID$LINEAGE2=="Alpha") #136
sum(GISAID$LINEAGE2=="Delta") #683
sum(GISAID$LINEAGE2=="B.1.625") #207
...
VOCs = c("Lambda", sel_target_VOC,"Gamma","Alpha", "Delta", "B.1.625")
...
remove2 = remove2[!(remove2 %in% c("Lambda", sel_target_VOC,"Gamma","Alpha", "Delta", "B.1.625"))]
GISAID_sel$LINEAGE2[(GISAID_sel$LINEAGE2 %in% remove2)] = "other" # minority VOCs
GISAID_sel$LINEAGE2 = factor(GISAID_sel$LINEAGE2)
GISAID_sel$LINEAGE2 = relevel(GISAID_sel$LINEAGE2, ref="Gamma") # we code UK strain as the reference level
levels(GISAID_sel$LINEAGE2)
# [1] "B.1.1.7"                  "B.1.1.519"                "B.1.351"                  "B.1.617.1"                "C.1.2"                   
# [6] "Delta (B.1.617.2 & AY.X)" "other" 
levels_LINEAGE2 = c("Lambda", sel_target_VOC,"Gamma","Alpha", "Delta", "B.1.625")
...
lineage_cols2[which(levels(GISAID_sel$LINEAGE2)=="Alpha")] = "#DC0000FF"
lineage_cols2[which(levels(GISAID_sel$LINEAGE2)=="Lambda")] = "#3C5488FF"
lineage_cols2[which(levels(GISAID_sel$LINEAGE2)=="Gamma")] = "#00A087FF"
lineage_cols2[which(levels(GISAID_sel$LINEAGE2)=="Delta")] = "#4DBBD5B2"
lineage_cols2[which(levels(GISAID_sel$LINEAGE2)=="B.1.625")] = "#632cc9"
lineage_cols2[which(levels(GISAID_sel$LINEAGE2)==sel_target_VOC)] = "#F39B7FFF"
...
muller_colombia_raw = ggplot(data=data_agbyweek2, aes(x=collection_date, y=count, group=LINEAGE2)) +
  ...
  scale_x_continuous(breaks=as.Date(c("2021-03-01","2021-04-01","2021-05-01","2021-06-01","2021-07-01")),
                     labels=substring(months(as.Date(c("2021-03-01","2021-04-01","2021-05-01","2021-06-01","2021-07-01"))),1,1),
...
ggsave(filename = "Muller_prev3.jpg", plot = muller_colombia_raw, height = 20, width = 25, units = "cm", dpi = 900)
...
# df      BIC
# fit1_colombia_multi 10 5487.111
# fit2_colombia_multi 15 5479.911
# fit3_colombia_multi 20 5511.115
# fit4_colombia_multi 25 5544.903

# growth rate advantage compared to Delta in overall model (difference in growth rate per day) 
emtrcolombia = emtrends(fit2_colombia_multi, trt.vs.ctrl ~ LINEAGE2,  
...
# 1  Lambda - Mu -0.01366645 0.0017859151 NA -0.01716678 -0.01016612  1.973403e-14
# 2   Gamma - Mu -0.02203853 0.0008969259 NA -0.02379648 -0.02028059 2.568215e-133
# 3   Alpha - Mu -0.04383231 0.0031998831 NA -0.05010396 -0.03756065  1.042209e-42
# 4   Delta - Mu  0.05948059 0.0028814183 NA  0.05383312  0.06512807  1.132630e-94
# 5 B.1.625 - Mu -0.03405447 0.0020667524 NA -0.03810523 -0.03000371  5.343149e-61

# fitted prop of different LINEAGES in Colombiafor 2020-09-01
multinom_preds_today_avg = data.frame(emmeans(fit2_colombia_multi, ~ LINEAGE2|1,
...
extrapolate = 30
date.from = as.numeric(as.Date("2021-03-07"))
date.to = as.numeric(as.Date("2021-10-01")) # max(GISAID_sel$DATE_NUM)+extrapolate
...
muller_colombia_mfit = ggplot(data=fit_colombia_multi_preds,
  ...
  scale_x_continuous(breaks=as.Date(c("2021-03-01","2021-04-01","2021-05-01","2021-06-01","2021-07-01","2021-08-01","2021-09-01","2021-10-01")),
                     labels=substring(months(as.Date(c("2021-03-01","2021-04-01","2021-05-01","2021-06-01","2021-07-01","2021-08-01","2021-09-01","2021-10-01"))),1,1),
...
ggsave("muller_plot_pred3.jpg", plot = muller_colombia_mfit, height = 20, width = 25, units = "cm", dpi = 900)
...
fit_colombia_multi_preds_withCI_overall = data.frame(emmeans(fit2_colombia_multi,
                                                             ~ LINEAGE2,
                                                             by=c("DATE_NUM"),
...
ggsave("muller_pred_smoothed3.jpg", plot = muller_colombia_mfit2, width = 25, height = 20, units = "cm", dpi = 900)
...
plot_colombia_mfit_logit = qplot(data=fit_colombia_multi_preds2, x=collection_date, y=prob, geom="blank") +
  ...
  coord_cartesian(xlim=c(as.Date("2021-03-01"),as.Date(date.to, origin="1970-01-01")), ylim=c(0.001, 0.9901), expand=c(0,0))
plot_colombia_mfit_logit

ggsave("mnomial_logit3.jpg", plot = plot_colombia_mfit_logit, height = 20, width = 25, units = "cm", dpi = 900)

# on response scale:
plot_colombia_mfit3 = qplot(data=fit_colombia_multi_preds2, x=collection_date, y=100*prob, geom="blank") +
  # facet_wrap(~ STATE) +
  ...
  ylab("Share (%)") +
  theme_minimal() + xlab("") +
  scale_x_continuous(breaks=as.Date(c("2021-03-01","2021-04-01","2021-05-01","2021-06-01","2021-07-01","2021-08-01")),
                     labels=substring(months(as.Date(c("2021-03-01","2021-04-01","2021-05-01","2021-06-01","2021-07-01", "2021-08-01"))),1,1),
                     limits=as.Date(c("2021-03-01",NA)), expand=c(0,0)) +
...
ggsave("mnomial_response3.jpg", plot = plot_colombia_mfit, height = 20, width = 25, units = "cm", dpi = 900)
ggsave("mnomial_response3.svg", plot = plot_colombia_mfit, height = 20, width = 25, units = "cm", dpi = 900)
...
cases_tot <- filter(cases_tot, between(date, as.Date("2021-03-01"), as.Date("2021-10-01")))
cases_tot = cases_tot[cases_tot$date>=as.Date("2021-03-01"),]
...
fitted_cases_raw
ggsave("fitted_cases_raw3.jpg", plot = fitted_cases_raw, height = 20, width = 25, units = "cm", dpi = 900)
...
fitted_cases_smoothed <- ggplot(data=fit_colombia_multi_preds_withCI_overall[fit_colombia_multi_preds_withCI_overall$collection_date>=as.Date("2020-09-01")&
  ...
  scale_fill_manual("variant", values=lineage_cols2) +
  scale_colour_manual("variant", values=lineage_cols2) +
  coord_cartesian(xlim=c(as.Date("2021-03-01"),max(cases_tot$date)))

fitted_cases_smoothed
ggsave("fitted_cases_smoothed3.jpg", plot = fitted_cases_smoothed, height = 20, width = 25, units = "cm", dpi = 900)
ggsave("fitted_cases_smoothed3.svg", plot = fitted_cases_smoothed, height = 20, width = 25, units = "cm", dpi = 900)
...
avg_R <- qplot(data=avg_r_cases, x=DATE-7, y=Re, ymin=Re_LOWER, ymax=Re_UPPER, geom="ribbon", alpha=I(0.5), fill=I("steelblue")) +
  ...
  scale_x_continuous(breaks=as.Date(c("2021-03-01","2021-04-01","2021-05-01","2021-06-01","2021-07-01")),
                     labels=c("Mar 2021","Apr 2021","May 2021","Jun 2021","Jul 2021","Aug 2021")) +
  scale_x_date(date_labels = "%b-%Y")
...
ggsave("Re3.jpg", plot = avg_R, width = 25, height = 20, units = "cm", dpi = 900)
...
r_variants3 <- qplot(data=above_avg_r_variants2[!((above_avg_r_variants2$variant %in% c("other"))|above_avg_r_variants2$collection_date>max(cases_tot$DATE)),], 
  ...
  scale_x_continuous(breaks=as.Date(c("2021-03-01","2021-04-01","2021-05-01","2021-06-01","2021-07-01", "2021-08-01", "2021-09-01")),
                     labels=c("Mar 2021","Apr 2021","May 2021","Jun 2021","Jul 2021", "Aug 2021", "Sep 2021")) +
...
ggsave("R_variants3.jpg", plot = r_variants3, width = 25, height = 20, units = "cm", dpi = 1200)

ggsave("R_variants3.svg", plot = r_variants3, width = 25, height = 20, units = "cm", dpi = 1200)
...
emtrcol_pairw = emtrends(fit2_colombia_multi, pairwise ~ LINEAGE2,  
                         var="DATE_NUM",  mode="latent",
...
gtsave(r_gt, filename = "r_table3.pdf", expand = 10, path = "~/Documents/R scripts/Phylodynamics/Multinomial fit databases")
gtsave(multinom_gt, filename = "mnom_table3.pdf", expand = 10, path = "~/Documents/R scripts/Phylodynamics/Multinomial fit databases")
gtsave(pairwise_r, filename = "pwise_r_table3.pdf", expand = 10, path = "~/Documents/R scripts/Phylodynamics/Multinomial fit databases")
