...
GISAID = read.csv("~/Documents/R scripts/Phylodynamics/Multinomial fit databases/2nd/GISAID2_upd.csv", sep = ",")
...
range(GISAID$date) # "2020-09-02" "2021-03-01"
nrow(GISAID) # 430
...
nrow(GISAID_sel) # 430
range(GISAID_sel$date) #  "2020-09-02" "2021-03-01"
...
sum(GISAID_sel$LINEAGE2==sel_target_VOC) # 31
sum(GISAID_sel$LINEAGE2=="B.1.420") # 25
sum(GISAID_sel$LINEAGE2=="B.1.1.348")# 55
sum(GISAID$LINEAGE2=="B.1") #88
sum(GISAID$LINEAGE2=="B.1.111") #88
...
VOCs = c("B.1", sel_target_VOC,"B.1.111","B.1.1", "B.1.420", "B.1.1.348")
...
remove2 = remove2[!(remove2 %in% c("B.1", sel_target_VOC,"B.1.111","B.1.1", "B.1.420", "B.1.1.348"))]
...
levels_LINEAGE2 = c("B.1", sel_target_VOC,"B.1.111","B.1.1", "B.1.420", "B.1.1.348")
...
lineage_cols2[which(levels(GISAID_sel$LINEAGE2)=="B.1.420")] = "magenta"
lineage_cols2[which(levels(GISAID_sel$LINEAGE2)=="B.1.1.348")] = "#91D1C2FF"
lineage_cols2[which(levels(GISAID_sel$LINEAGE2)=="B.1.1")] = "#314f39"
lineage_cols2[which(levels(GISAID_sel$LINEAGE2)==sel_target_VOC)] = "#00A087FF"
...
muller_colombia_raw = ggplot(data=data_agbyweek2, aes(x=collection_date, y=count, group=LINEAGE2)) +
  ...
  scale_x_continuous(breaks=as.Date(c("2020-08-01","2020-09-01","2020-10-01","2020-11-01","2020-12-01","2021-01-01","2021-02-01","2021-03-01")),
                     labels=substring(months(as.Date(c("2020-09-01","2020-10-01","2020-11-01","2020-12-01","2021-01-01","2021-02-01","2021-03-01"))),1,1),
                     limits=as.Date(c("2020-09-01",NA)), 
                     expand=c(0,0)) +
...
ggsave(filename = "Muller_prev2.jpg", plot = muller_colombia_raw, height = 20, width = 25, units = "cm", dpi = 900)
...
library(splines)
set.seed(1)
fit1_colombia_multi = nnet::multinom(LINEAGE2 ~ scale(DATE_NUM), weights=count, data=data_agbyweek2, maxit=10000)
fit2_colombia_multi = nnet::multinom(LINEAGE2 ~ ns(DATE_NUM, df=2), weights=count, data=data_agbyweek2, maxit=10000)
fit3_colombia_multi = nnet::multinom(LINEAGE2 ~ ns(DATE_NUM, df=3), weights=count, data=data_agbyweek2, maxit=10000)
fit4_colombia_multi = nnet::multinom(LINEAGE2 ~ ns(DATE_NUM, df=4), weights=count, data=data_agbyweek2, maxit=10000)
BIC(fit1_colombia_multi, fit2_colombia_multi, fit3_colombia_multi, fit4_colombia_multi) 
# df      BIC
# fit1_colombia_multi 10 1074.262
# fit2_colombia_multi 25 1120.207
# fit3_colombia_multi 20 1097.566
# fit4_colombia_multi 20 1095.907
...
# 1       B.1 - Gamma -0.05380351 0.009969650 NA -0.07334366 -0.03426335 6.786646e-08
# 2   B.1.111 - Gamma -0.05183730 0.009951685 NA -0.07134224 -0.03233235 1.899668e-07
# 3     B.1.1 - Gamma -0.05168548 0.010599941 NA -0.07246098 -0.03090998 1.082499e-06
# 4   B.1.420 - Gamma -0.06554728 0.010723897 NA -0.08656573 -0.04452883 9.822834e-10
# 5 B.1.1.348 - Gamma -0.03792207 0.010218735 NA -0.05795043 -0.01789372 2.064142e-04
...
# 1     Gamma 0.49783047 0.09783320 NA 0.3060809282 0.68958002
# 2       B.1 0.11601243 0.03336061 NA 0.0506268448 0.18139802
# 3   B.1.111 0.13270412 0.03730223 NA 0.0595930871 0.20581516
# 4     B.1.1 0.05179405 0.02047969 NA 0.0116545985 0.09193351
# 5   B.1.420 0.01291335 0.00645678 NA 0.0002582941 0.02556841
# 6 B.1.1.348 0.18874557 0.05421389 NA 0.0824882938 0.29500284
# % non-B.1.421
...
date.to = as.numeric(as.Date("2021-03-01")) # max(GISAID_sel$DATE_NUM)+extrapolate
...
muller_colombia_mfit = ggplot(data=fit_colombia_multi_preds,
  ...
  scale_x_continuous(breaks=as.Date(c("2020-08-01","2020-09-01","2020-10-01","2020-11-01","2020-12-01","2021-01-01","2021-02-01","2021-03-01")),
                     labels=substring(months(as.Date(c("2020-08-01","2020-09-01","2020-10-01","2020-11-01","2020-12-01","2021-01-01","2021-02-01","2021-03-01"))),1,1),
...
ggsave("muller_plot_pred2.jpg", plot = muller_colombia_mfit, height = 20, width = 25, units = "cm", dpi = 900)

#
#
library(ggpubr)
ggarrange(muller_colombia_raw + coord_cartesian(xlim=c(as.Date("2020-09-01"),as.Date(date.to, origin="1970-01-01")))+
...
muller_colombia_mfit2 = ggplot(data=fit_colombia_multi_preds_withCI_overall, 
  ...
  annotate("rect", xmin=max(GISAID_sel$DATE_NUM)+1, 
           xmax=as.Date(date.to, origin="1970-01-01"), ymin=0, ymax=1, alpha=0.4, fill="white") + # extrapolated part
  scale_x_continuous(breaks=as.Date(c("2020-09-01","2020-10-01","2020-11-01","2020-12-01","2021-01-01","2021-02-01","2021-03-01","2021-04-01")),
                     labels=substring(months(as.Date(c("2020-09-01","2020-10-01","2020-11-01","2020-12-01","2021-01-01","2021-02-01","2021-03-01","2021-04-01"))),1,1),
                     limits=as.Date(c("2020-09-01",NA)), expand=c(0,0)) +
  # guides(color = guide_legend(reverse=F, nrow=1, byrow=T), fill = guide_legend(reverse=F, nrow=1, byrow=T)) +
  theme_minimal() + theme(legend.position="right", 
                          axis.title.x=element_blank()) + 
...
ggsave("muller_pred_smoothed2.jpg", plot = muller_colombia_mfit2, width = 25, height = 20, units = "cm", dpi = 900)
...
plot_colombia_mfit_logit = qplot(data=fit_colombia_multi_preds2, x=collection_date, y=prob, geom="blank") +
  ...
  coord_cartesian(xlim=c(as.Date("2020-09-01"),as.Date(date.to, origin="1970-01-01")), ylim=c(0.001, 0.9901), expand=c(0,0))
  plot_colombia_mfit_logit

ggsave("mnomial_logit2.jpg", plot = plot_colombia_mfit_logit, height = 20, width = 25, units = "cm", dpi = 900)

# on response scale:
plot_colombia_mfit2 = qplot(data=fit_colombia_multi_preds2, x=collection_date, y=100*prob, geom="blank") +
  # facet_wrap(~ STATE) +
...
ggsave("mnomial_response2.svg", plot = plot_colombia_mfit2, height = 20, width = 25, units = "cm", dpi = 1200)
ggsave("mnomial_response2.svg", plot = plot_colombia_mfit2, height = 20, width = 25, units = "cm", dpi = 1200)
...
cases_tot <- filter(cases_tot, between(date, as.Date("2020-08-10"), as.Date("2021-03-06")))
...
fitted_cases_raw <- ggplot(data=fit_colombia_multi_preds_withCI_overall[fit_colombia_multi_preds_withCI_overall$collection_date>=as.Date("2020-09-01"),], 
                           aes(x=collection_date, y=cases, group=LINEAGE2)) + 
  # facet_wrap(~ REGION, scale="free", ncol=3) +
  geom_area(aes(lwd=I(1.2), colour=NULL, fill=LINEAGE2, group=LINEAGE2), position="stack") +
  scale_x_continuous(breaks=as.Date(c("2020-08-01","2020-09-01","2020-10-01","2020-11-01","2020-12-01","2021-01-01","2021-02-01","2021-03-01")),
                     labels=substring(months(as.Date(c("2020-08-01","2020-09-01","2020-10-01","2020-11-01","2020-12-01","2021-01-01","2021-02-01","2021-03-01"))),1,1),
                     limits=c(as.Date("2020-08-01"),max(cases_tot$date)), 
                     expand=c(0,0)) +
  # guides(color = guide_legend(reverse=F, nrow=1, byrow=T), fill = guide_legend(reverse=F, nrow=1, byrow=T)) +
  theme_minimal() + theme(legend.position="right") + 
...
ggsave("fitted_cases_raw2.jpg", plot = fitted_cases_raw, height = 20, width = 25, units = "cm", dpi = 900)

fitted_cases_smoothed <- ggplot(data=fit_colombia_multi_preds_withCI_overall[fit_colombia_multi_preds_withCI_overall$collection_date>=as.Date("2020-09-01")&
  ...
  scale_x_continuous(breaks=as.Date(c("2020-09-01","2020-10-01","2020-11-01","2020-12-01","2021-01-01","2021-02-01","2021-03-01","2021-04-01")),
                     labels=substring(months(as.Date(c("2020-09-01","2020-10-01","2020-11-01","2020-12-01","2021-01-01","2021-02-01","2021-03-01","2021-04-01"))),1,1),
                     limits=c(as.Date("2020-09-01"),today), 
                     expand=c(0,0)) +
  # guides(color = guide_legend(reverse=F, nrow=1, byrow=T), fill = guide_legend(reverse=F, nrow=1, byrow=T)) +
  theme_minimal() + theme(legend.position="right") + 
  ...
  scale_fill_manual("variant", values=lineage_cols2) +
  scale_colour_manual("variant", values=lineage_cols2) +
  coord_cartesian(xlim=c(as.Date("2020-09-01"),max(cases_tot$date)))

ggsave("fitted_cases_smoothed2.jpg", plot = fitted_cases_smoothed, height = 20, width = 25, units = "cm", dpi = 900)

ggsave("fitted_cases_smoothed2.svg", plot = fitted_cases_smoothed, height = 20, width = 25, units = "cm", dpi = 900)
...
avg_R <- qplot(data=avg_r_cases, x=DATE-7, y=Re, ymin=Re_LOWER, ymax=Re_UPPER, geom="ribbon", alpha=I(0.5), fill=I("steelblue")) +
  ...
  scale_x_continuous(breaks=as.Date(c("2020-08-01","2020-09-01","2020-10-01","2020-11-01","2020-12-01","2021-01-01","2021-02-01","2021-03-01")),
                     labels=c("Sep 2020","Oct 2020","Nov 2020","Dic 2020","Jan 2021","Feb 2021","Mar 2021", "Apr 2021")) +
 ...
 ggsave("Re2.jpg", plot = avg_R, width = 25, height = 20, units = "cm", dpi = 900)
...
r_variants2 <- qplot(data=above_avg_r_variants2[!((above_avg_r_variants2$variant %in% c("other"))|above_avg_r_variants2$collection_date>max(cases_tot$DATE)),], 
...
ggsave("R_variants2.jpg", plot = r_variants2, width = 25, height = 20, units = "cm", dpi = 1200)

ggsave("R_variants2.svg", plot = r_variants2, width = 25, height = 20, units = "cm", dpi = 1200)
...
gtsave(r_gt, filename = "r_table2.pdf", expand = 10, path = "~/Documents/R scripts/Phylodynamics/Multinomial fit databases")
gtsave(multinom_gt, filename = "mnom_table2.pdf", expand = 10, path = "~/Documents/R scripts/Phylodynamics/Multinomial fit databases")
gtsave(pairwise_r, filename = "pwise_r_table2.pdf", expand = 10, path = "~/Documents/R scripts/Phylodynamics/Multinomial fit databases")
